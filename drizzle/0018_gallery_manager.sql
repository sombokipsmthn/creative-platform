...
| Gallery Manager V2
|--------------------------------------------------------------------------
| Adds expiry, client sessions/actions, proofing comments, approval,
| download presets/tracking, watermark settings and processed variants.
|--------------------------------------------------------------------------
*/
...
WHERE NOT EXISTS (
  SELECT 1
  FROM gallery_watermarks w
  WHERE w.gallery_id = galleries.id
);
...
WHERE NOT EXISTS (
  SELECT 1
  FROM gallery_download_presets p
  WHERE p.gallery_id = g.id
);

CREATE INDEX IF NOT EXISTS galleries_expires_at_idx
