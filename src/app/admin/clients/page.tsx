...
>>>>>>> 1db1f5519c16c9bd97c5343408a059030ddd0b39
...
const emptyForm = { name: '', email: '', phone: '', company: '', website: '', notes: '', status: 'active' as ClientStatus };
...
function statusClass(status: string) {
  if (status === 'active') return 'bg-emerald-500/20 border-emerald-500/30 text-emerald-700 dark:text-emerald-300';
  if (status === 'archived') return 'bg-slate-100 dark:bg-zinc-800 border-slate-300 dark:border-zinc-700 text-slate-600 dark:text-zinc-400';
...
=======
  ...
  useEffect(() => {
    ...
    async function loadClients() {
      try {
        ...
        console.error('Failed to load clients:', err);
        if (!cancelled) setError('Unable to load clients. Please try again.');
      } finally {
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter((client) => {
      const matchesSearch = !query || [client.name, client.company, client.email, client.phone, client.website].filter(Boolean).join(' ').toLowerCase().includes(query);
...
=======
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 dark:border-zinc-800/80 pb-6">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-500 font-mono mt-2">Database-backed client records and workflow status.</p>
          </div>
          <button onClick={openCreateModal} className="px-5 py-2.5 btn-primary text-xs font-mono uppercase tracking-widest rounded-lg flex items-center gap-2 shadow-sm">+ Register New Client</button>
        </div>

        {error && !isAddClientOpen && !selectedClient && <div className="p-4 rounded-xl border border-red-500/30 bg-red-500/10 text-sm text-red-700 dark:text-red-300">{error}</div>}
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-100 dark:bg-zinc-900/40 p-4 border border-slate-200 dark:border-zinc-800/80 rounded-2xl">
          <input type="text" placeholder="Search clients by name, company, email, phone..." value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="w-full md:w-96 px-4 py-2 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 text-xs font-mono text-slate-900 dark:text-white rounded-xl focus:border-purple-600 focus:outline-none" />
          <div className="flex flex-wrap gap-2 text-xs font-mono">
            {([['ALL', clients.length], ['active', activeCount], ['inactive', inactiveCount], ['archived', archivedCount]] as const).map(([status, count]) => (
              <button key={status} onClick={() => setFilterStatus(status)} className={`px-3 py-1.5 rounded-lg border transition-all ${filterStatus === status ? 'bg-purple-600 border-purple-500 text-white' : 'btn-secondary'}`}>
        ...
        {loading ? (
          <div className="p-12 text-center border border-slate-200 dark:border-zinc-800 rounded-2xl bg-white dark:bg-zinc-950/40"><p className="text-xs font-mono text-slate-500 dark:text-zinc-500 uppercase tracking-widest">Loading clients…</p></div>
        ) : filteredClients.length === 0 ? (
          <div className="p-12 text-center border border-dashed border-slate-300 dark:border-zinc-800 rounded-2xl">
            ...
            <button onClick={openCreateModal} className="mt-4 px-4 py-2 btn-primary text-xs font-mono rounded-lg">Register your first client</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {filteredClients.map((client) => (
              <div key={client.id} className="p-6 border border-slate-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/20 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:border-purple-600/50 transition-all group shadow-sm dark:shadow-none">
                <div className="space-y-3 max-w-3xl">
                  <div className="flex flex-wrap items-center gap-2">
                    {client.company && <span className="px-2.5 py-0.5 bg-purple-600/20 border border-purple-500/30 text-purple-700 dark:text-purple-300 text-[10px] font-mono rounded-full uppercase">{client.company}</span>}
                    <span className={`px-2.5 py-0.5 border text-[10px] font-mono rounded-full uppercase ${statusClass(client.status)}`}>{statusLabel(client.status)}</span>
                  </div>
                  <h3 className="text-xl font-medium text-slate-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{client.name}</h3>
                  ...
                  {client.notes && <p className="text-xs text-slate-500 dark:text-zinc-500 font-light italic">“{client.notes}”</p>}
                  <p className="text-[10px] text-slate-400 dark:text-zinc-600 font-mono uppercase">Added {formatDate(client.createdAt)}</p>
                </div>
                <button onClick={() => { setError(''); setSelectedClient(client); }} className="px-4 py-2.5 btn-secondary text-xs font-mono rounded-xl cursor-pointer">View / Edit</button>
              </div>
            ))}
          </div>
        )}
...
=======
              ...
              <div className="space-y-1"><p className="text-xs font-mono uppercase text-purple-600 dark:text-purple-400 tracking-widest">CRM</p><h2 className="text-2xl font-light">Register New Client</h2></div>
              ...
              <form onSubmit={handleCreateClient} className="space-y-4">
                <div className="space-y-1"><label className="text-[10px] font-mono text-slate-600 dark:text-zinc-400 uppercase">Contact Name *</label><input required type="text" placeholder="e.g. Jane Wanjiku" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="w-full px-4 py-2.5 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-xs font-mono rounded-lg focus:border-purple-600 focus:outline-none" /></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1"><label className="text-[10px] font-mono text-slate-600 dark:text-zinc-400 uppercase">Email</label><input type="email" placeholder="jane@company.com" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} className="w-full px-4 py-2.5 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-xs font-mono rounded-lg focus:border-purple-600 focus:outline-none" /></div>
                  <div className="space-y-1"><label className="text-[10px] font-mono text-slate-600 dark:text-zinc-400 uppercase">Phone</label><input type="text" placeholder="+254 700 000 000" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} className="w-full px-4 py-2.5 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-xs font-mono rounded-lg focus:border-purple-600 focus:outline-none" /></div>
>>>>>>> 1db1f5519c16c9bd97c5343408a059030ddd0b39
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
<<<<<<< HEAD
...
=======
                  <div className="space-y-1"><label className="text-[10px] font-mono text-slate-600 dark:text-zinc-400 uppercase">Company / Organization</label><input type="text" placeholder="e.g. Safaricom" value={form.company} onChange={(event) => setForm({ ...form, company: event.target.value })} className="w-full px-4 py-2.5 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-xs font-mono rounded-lg focus:border-purple-600 focus:outline-none" /></div>
                  <div className="space-y-1"><label className="text-[10px] font-mono text-slate-600 dark:text-zinc-400 uppercase">Website</label><input type="url" placeholder="https://company.com" value={form.website} onChange={(event) => setForm({ ...form, website: event.target.value })} className="w-full px-4 py-2.5 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-xs font-mono rounded-lg focus:border-purple-600 focus:outline-none" /></div>
                </div>
                <div className="space-y-1"><label className="text-[10px] font-mono text-slate-600 dark:text-zinc-400 uppercase">Status</label><select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as ClientStatus })} className="w-full px-4 py-2.5 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-xs font-mono rounded-lg focus:border-purple-600 focus:outline-none"><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select></div>
                <div className="space-y-1"><label className="text-[10px] font-mono text-slate-600 dark:text-zinc-400 uppercase">Internal Notes</label><textarea rows={3} placeholder="Project, billing, or relationship notes..." value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} className="w-full px-4 py-2 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-xs font-mono rounded-lg focus:border-purple-600 focus:outline-none" /></div>
                <button type="submit" disabled={saving} className="w-full py-3.5 btn-primary text-xs font-mono uppercase tracking-widest rounded-lg transition-colors shadow-md disabled:opacity-50">{saving ? 'Saving Client…' : '+ Add Client to CRM'}</button>
>>>>>>> 1db1f5519c16c9bd97c5343408a059030ddd0b39
              </form>
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-center justify-center p-6 text-slate-900 dark:text-white">
            <div className="max-w-xl w-full p-8 border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-2xl space-y-6 relative shadow-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-2 border-b border-slate-200 dark:border-zinc-800 pb-5">
                ...
                <p className="text-xs font-mono text-slate-600 dark:text-zinc-400 break-words">{[selectedClient.email, selectedClient.phone, selectedClient.website].filter(Boolean).join(' • ') || 'No contact details'}</p>
              </div>
              {error && <div className="p-3 rounded-lg border border-red-500/30 bg-red-500/10 text-xs text-red-700 dark:text-red-300">{error}</div>}
              <div className="p-6 bg-slate-100 dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800 rounded-xl space-y-4">
                <div><p className="text-[10px] font-mono text-purple-600 dark:text-purple-400 uppercase">Client Status</p><p className="text-xs text-slate-500 dark:text-zinc-500 mt-1">Changes are saved directly to the database.</p></div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  {(['active', 'inactive', 'archived'] as ClientStatus[]).map((status) => (
                    <button key={status} disabled={updating || selectedClient.status === status} onClick={() => updateClientStatus(status)} className={`px-3 py-2.5 border rounded-lg text-[10px] font-mono uppercase transition-all disabled:opacity-50 ${selectedClient.status === status ? statusClass(status) : 'border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-slate-600 dark:text-zinc-400 hover:border-purple-500'}`}>{statusLabel(status)}</button>
                  ))}
>>>>>>> 1db1f5519c16c9bd97c5343408a059030ddd0b39
                </div>
...
=======
              <div className="space-y-2"><p className="text-[10px] font-mono text-slate-500 dark:text-zinc-500 uppercase">Internal Notes</p><p className="text-sm text-slate-700 dark:text-zinc-300 whitespace-pre-wrap">{selectedClient.notes || 'No internal notes.'}</p></div>
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-zinc-800 text-[10px] font-mono uppercase text-slate-400 dark:text-zinc-600"><div>Created {formatDate(selectedClient.createdAt)}</div><div className="text-right">Updated {formatDate(selectedClient.updatedAt)}</div></div>
            </div>
          </div>
        )}
>>>>>>> 1db1f5519c16c9bd97c5343408a059030ddd0b39
