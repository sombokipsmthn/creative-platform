...
function StatCard({ label, value, detail, href, icon }: { label: string; value: string; detail: string; href: string; icon: React.ReactNode }) {
  return (
    <Link href={href} className="group rounded-2xl border border-slate-200 bg-white p-5 transition hover:-translate-y-0.5 hover:border-purple-300 hover:shadow-lg dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <p className="mt-5 text-[10px] font-sans uppercase tracking-[0.2em] text-slate-400">{label}</p>
...
function Panel({ title, eyebrow, children }: { title: string; eyebrow?: string; children: React.ReactNode }) {
  return <section className="rounded-2xl border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-950"><div className="border-b border-slate-100 px-5 py-4 dark:border-zinc-900">{eyebrow && <p className="text-[9px] font-sans uppercase tracking-[0.22em] text-purple-600 dark:text-purple-400">{eyebrow}</p>}<h2 className="mt-1 text-base font-semibold text-slate-900 dark:text-white">{title}</h2></div>{children}</section>;
}
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#09090b] dark:text-zinc-100">
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        {loading && !stats ? <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-36 animate-pulse rounded-2xl border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-950" />)}</div> : stats ? <>
          <section><div className="mb-4"><p className="text-[9px] font-sans uppercase tracking-[0.22em] text-purple-600 dark:text-purple-400">Business Overview</p><h2 className="mt-1 text-xl font-semibold">Performance {rangeLabels[range]}</h2></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <section className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-[1.35fr_0.65fr]">
            ...
            <Panel title="Quote pipeline" eyebrow="Commercial health"><div className="space-y-5 px-5 py-5">{[['Draft', stats.quotes.statuses.draft ?? 0], ['Sent', stats.quotes.statuses.sent ?? 0], ['Accepted', stats.quotes.statuses.accepted ?? 0], ['Rejected', stats.quotes.statuses.rejected ?? 0], ['Invoiced', stats.quotes.statuses.invoiced ?? 0]].map(([label, value]) => <div key={label as string}><div className="mb-1.5 flex items-center justify-between text-xs"><span className="text-slate-500">{label}</span><span className="font-medium text-slate-800 dark:text-zinc-200">{value}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-zinc-900"><div className="h-full rounded-full bg-purple-500" style={{ width: `${Math.min(100, ((Number(value) || 0) / Math.max(stats.overview.quotes, 1)) * 100)}%` }} /></div></div>)}<div className="border-t border-slate-100 pt-4 dark:border-zinc-900"><div className="flex items-end justify-between"><div><p className="text-[9px] font-sans uppercase tracking-widest text-slate-400">Conversion</p><p className="mt-1 text-2xl font-semibold">{stats.quotes.conversionRate}%</p></div><p className="text-xs text-slate-500">Accepted / decided quotes</p></div></div></div></Panel>
          </section>

          <section className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-2">
            <Panel title="Recent activity" eyebrow="Across your workspace">{stats.activity.length === 0 ? <div className="px-5 py-10 text-center text-sm text-slate-500">No recent activity yet.</div> : <div className="divide-y divide-slate-100 dark:divide-zinc-900">{stats.activity.map((item) => <div key={item.id} className="flex items-center gap-3 px-5 py-3.5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-50 text-slate-500 dark:bg-zinc-900 dark:text-zinc-400">{getActivityIcon(item.type)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-800 dark:text-zinc-200">{item.title}</p><p className="truncate text-xs text-slate-500">{item.description}</p></div><span className="shrink-0 text-[10px] font-sans text-slate-400">{formatDate(item.date)}</span></div>)}</div>}</Panel>
            <Panel title="Workspace snapshot" eyebrow="Platform totals"><div className="grid grid-cols-2 divide-x divide-y divide-slate-100 dark:divide-zinc-900"><div className="p-5"><p className="text-[9px] font-sans uppercase tracking-widest text-slate-400">Invoiced</p><p className="mt-2 text-xl font-semibold">{formatMoney(stats.finance.periodInvoicedValue)}</p><p className="mt-1 text-xs text-slate-500">{rangeLabels[range]}</p></div><div className="p-5"><p className="text-[9px] font-sans uppercase tracking-widest text-slate-400">Paid</p><p className="mt-2 text-xl font-semibold">{formatMoney(stats.finance.periodPaidValue)}</p><p className="mt-1 text-xs text-slate-500">{rangeLabels[range]}</p></div><div className="p-5"><p className="text-[9px] font-sans uppercase tracking-widest text-slate-400">New clients</p><p className="mt-2 text-xl font-semibold">{stats.overview.newClients}</p><p className="mt-1 text-xs text-slate-500">{rangeLabels[range]}</p></div><div className="p-5"><p className="text-[9px] font-sans uppercase tracking-widest text-slate-400">New projects</p><p className="mt-2 text-xl font-semibold">{stats.overview.newProjects}</p><p className="mt-1 text-xs text-slate-500">{rangeLabels[range]}</p></div></div></Panel>
          </section>

          <section className="mt-8 rounded-3xl border border-purple-500/20 bg-purple-50 p-6 dark:bg-purple-950/20 md:p-8"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[9px] font-sans font-semibold uppercase tracking-[0.22em] text-purple-600 dark:text-purple-400">Quick actions</p><h2 className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">Keep the work moving.</h2><p className="mt-1 text-sm text-slate-600 dark:text-zinc-400">Jump straight into the parts of the platform you use most.</p></div><div className="flex flex-wrap gap-2"><Link href="/admin/quotes/new" className="rounded-full bg-purple-600 px-4 py-2.5 text-[10px] font-sans uppercase tracking-widest text-white transition hover:bg-purple-700">New Quote</Link><Link href="/admin/clients" className="rounded-full border border-purple-200 bg-white px-4 py-2.5 text-[10px] font-sans uppercase tracking-widest text-purple-700 transition hover:border-purple-300 dark:border-purple-900 dark:bg-zinc-950 dark:text-purple-300">Add Client</Link><Link href="/admin/projects" className="rounded-full border border-purple-200 bg-white px-4 py-2.5 text-[10px] font-sans uppercase tracking-widest text-purple-700 transition hover:border-purple-300 dark:border-purple-900 dark:bg-zinc-950 dark:text-purple-300">Projects</Link></div></div></section>
        </> : null}
      </div>
    </main>
  );
