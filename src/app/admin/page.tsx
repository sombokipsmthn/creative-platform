      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        {loading && !stats ? <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-36 animate-pulse rounded-2xl border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-950" />)}</div> : stats ? <>
          <section><div className="mb-4"><p className="text-[9px] font-mono uppercase tracking-[0.22em] text-purple-600 dark:text-purple-400">Business Overview</p><h2 className="mt-1 text-xl font-semibold">Performance {rangeLabels[range]}</h2></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard label="Quoted Value" value={formatCurrency(stats.finance.periodQuotedValue)} detail={`${stats.overview.newQuotes} new quotes`} href="/admin/quotes" icon={<Wallet className="h-4 w-4" />} />
            <StatCard label="Paid Invoices" value={formatCurrency(stats.finance.periodPaidValue)} detail={`${invoicePaidRate}% paid across invoices`} href="/admin/invoices" icon={<CheckCircle2 className="h-4 w-4" />} />
            <StatCard label="Accepted Quotes" value={formatCurrency(stats.finance.acceptedQuoteValue)} detail={`${stats.quotes.conversionRate}% conversion rate`} href="/admin/quotes" icon={<FileCheck2 className="h-4 w-4" />} />
