...
export async function GET(request: Request) {
  try {
    ...
    const range = ["7d", "30d", "90d", "12m", "all"].includes(
      requestedRange
    )
      ? requestedRange
      : "30d";
    ...
    const [
      ...
      db
        .select({ value: count() })
        .from(clients)
        .where(eq(clients.creatorId, creator)),
      ...
      db
        ...
        .where(
          and(
            eq(clients.creatorId, creator),
            gte(clients.createdAt, periodStart),
            lt(clients.createdAt, periodEnd)
          )
        ),
      ...
      db
        .select({ value: count() })
        .from(projects)
        .where(eq(projects.creatorId, creator)),
      ...
      db
        .select({ value: count() })
        .from(quotes)
        .where(eq(quotes.creatorId, creator)),
      ...
      db
        ...
        .where(
          and(
            eq(quotes.creatorId, creator),
            gte(quotes.createdAt, periodStart),
            lt(quotes.createdAt, periodEnd)
          )
        ),
      ...
      db
        .select({ value: count() })
        .from(invoices)
        .where(eq(invoices.creatorId, creator)),
      ...
      db
        ...
        .where(
          and(
            ...
            lt(invoices.updatedAt, periodEnd)
          )
        ),
      ...
      db
        .select({ value: count() })
        .from(galleries)
        .where(eq(galleries.creatorId, creator)),
      ...
      db
        ...
        .where(
          and(
            eq(galleries.creatorId, creator),
            gte(galleries.createdAt, periodStart),
            lt(galleries.createdAt, periodEnd)
          )
        ),
    ...
    const invoiceStatuses = Object.fromEntries(
      invoiceStatusRows.map((row) => [row.status, Number(row.count)])
    );

    const galleryStatuses = Object.fromEntries(
      galleryStatusRows.map((row) => [row.status, Number(row.count)])
    );

    const acceptedQuotes = quoteStatuses.accepted ?? 0;
    const decisionQuotes =
      (quoteStatuses.sent ?? 0) +
      (quoteStatuses.accepted ?? 0) +
      (quoteStatuses.rejected ?? 0) +
      (quoteStatuses.invoiced ?? 0);

    const quoteConversionRate =
      decisionQuotes > 0
        ? Math.round((acceptedQuotes / decisionQuotes) * 100)
        : 0;
    ...
    const activity = [
      ...
      ...recentInvoices.map((item) => ({
        ...
        description: item.invoiceNumber || item.title,
        date: item.createdAt,
      })),
      ...recentGalleries.map((item) => ({
        id: `gallery-${item.id}`,
        type: "gallery" as const,
        title: `Gallery ${item.status}`,
        description: item.title,
        date: item.createdAt,
      })),
    ]
    ...
    return NextResponse.json({
      ...
      attention: {
        overdueInvoices: Number(overdueInvoices[0]?.value ?? 0),
        pendingQuotes:
          (quoteStatuses.sent ?? 0) + (quoteStatuses.draft ?? 0),
        activeProjects: Number(activeProjects[0]?.value ?? 0),
        activeGalleries:
          (galleryStatuses.active ?? 0) + (galleryStatuses.published ?? 0),
      },
      activity,
    });
  } catch (error) {
