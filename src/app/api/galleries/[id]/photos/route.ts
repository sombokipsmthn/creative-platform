...
async function ownsGallery(galleryId: string, creatorId: string) {
...
function dataUrlToBuffer(value: string) {
  ...
  if (buffer.byteLength > MAX_IMAGE_BYTES) {
    throw new Error("Image exceeds the 100MB upload limit.");
  }
  return { buffer, contentType: match[1] };
}

async function fetchRemoteImage(value: string) {
  const response = await fetch(value, { cache: "no-store", redirect: "follow" });
...
async function resolveImage(value: string) {
  if (value.startsWith("data:")) {
    const result = dataUrlToBuffer(value);
    if (!result) throw new Error("Invalid image data.");
    return result;
  }

  if (!/^https?:\/\//i.test(value)) {
    throw new Error("Photo URL must be an http(s) URL or a data URL.");
  }
...
function safeFilename(filename: string) {
  return filename.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 180) || "photo.jpg";
}

async function getWatermarkSettings(galleryId: string) {
  const result = await db.execute(sql`
  ...
  return {
    ...
    position: (row?.position || "bottom-right") as
      | "top-left"
      | "top-right"
      | "bottom-left"
      | "bottom-right"
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const watermark = await getWatermarkSettings(galleryId);

    for (const item of photosList) {
      const sourceUrl = item.url || item.displayUrl || item.originalUrl;
      if (!sourceUrl) continue;

      const source = await resolveImage(String(sourceUrl));
      const processed = await processImage(source.buffer, watermark);
      const photoId = crypto.randomUUID();
      const filename = safeFilename(String(item.filename || "photo.jpg"));
      ...
      if (result.rows[0]) insertedPhotos.push(result.rows[0]);
      currentSortOrder += 1;
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to add photos." },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (body.collectionId !== undefined) {
      await db.execute(sql`
        ...
        SET collection_id = ${body.collectionId || null}, updated_at = now()
    ...
    if (body.isHidden !== undefined) {
      await db.execute(sql`
        UPDATE gallery_photos
        SET is_hidden = ${Boolean(body.isHidden)}, updated_at = now()
        WHERE id = ${photoId} AND gallery_id = ${galleryId}
      `);
    }
    ...
    if (body.isFavorite !== undefined) {
      await db.execute(sql`
        UPDATE gallery_photos
        SET is_favorite = ${Boolean(body.isFavorite)}, updated_at = now()
        WHERE id = ${photoId} AND gallery_id = ${galleryId}
      `);
    }
    ...
    if (body.isSelected !== undefined) {
      await db.execute(sql`
        UPDATE gallery_photos
        SET is_selected = ${Boolean(body.isSelected)}, updated_at = now()
        WHERE id = ${photoId} AND gallery_id = ${galleryId}
      `);
    }
    ...
    if (body.isCover) {
      await db.execute(sql`
        UPDATE galleries
        SET cover_photo_id = ${photoId}, updated_at = now()
        WHERE id = ${galleryId} AND creator_id = ${creator.id}
      `);
    }
    ...
    console.error("PATCH gallery photo", error);
...
export async function DELETE(request: Request, context: Context) {
  try {
    ...
    const body = await request.json();
    const ids: string[] = Array.isArray(body.photoIds) ? body.photoIds : [];
    ...
    for (const photoId of ids) {
      const result = await db.execute(sql`
        SELECT original_path, display_path, thumbnail_path, watermark_path,
      ...
      for (const path of [
        ...
        if (path) {
          try {
            await storage.deleteObject(String(path));
          } catch (storageError) {
    ...
    console.error("DELETE gallery photos", error);
    return NextResponse.json({ error: "Unable to delete photos." }, { status: 500 });
  }
}
