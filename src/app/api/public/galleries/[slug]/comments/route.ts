...
export async function GET(_request: Request, context: Context) {
  try {
    ...
    const photoId = new URL(_request.url).searchParams.get("photoId");
    ...
    const comments = await db.execute(sql`
      ...
      WHERE gallery_id = ${gallery.id}
        AND (${photoId || null}::uuid IS NULL OR photo_id = ${photoId || null})
      ORDER BY created_at ASC
    `);

    return NextResponse.json({ comments: comments.rows, sessionId: session.id });
  } catch (error) {
    console.error("GET public gallery comments", error);
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    if (!photo.rows[0]) return NextResponse.json({ error: "Photo not found." }, { status: 404 });
    ...
    const result = await db.execute(sql`
      ...
      RETURNING id, photo_id, author_type, author_name, body, resolved_at, created_at
    `);

    return NextResponse.json({ comment: result.rows[0] }, { status: 201 });
  } catch (error) {
    console.error("POST public gallery comment", error);
