...
async function getPreset(
  ...
  return {
    ...
    format:
      (row?.format ||
        "jpg") as Preset["format"],
    include_watermark:
      Boolean(
        row?.include_watermark,
      ),
  };
}
...
async function getSource(
  ...
  return getGalleryStorage().getObject(
    isR2
      ? path || ""
      : url || "",
  );
...
export async function POST(
  ...
  try {
    ...
    for (
      const photo of downloadedPhotos
    ) {
      await db.execute(sql`
