  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={handleSave}
          className="px-4 py-2 text-xs font-sans uppercase tracking-widest bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-colors shadow-md shadow-purple-600/20"
        >
      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="fixed top-4 right-4 bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 px-4 py-2 rounded-xl text-xs font-sans shadow-lg"
          >
      ...
      <div className="relative">
        <div className="absolute top-0 left-0 right-0 text-center py-3 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm text-xs font-sans font-bold text-slate-900 dark:text-white">
          LIVE PREVIEW
        </div>
        ...
        <div className="mt-3 text-center text-xs text-slate-500 dark:text-zinc-400 font-sans">
      ...
      <div className="grid grid-cols-3 gap-2">
        {(['top-left', 'top-right', 'bottom-left', 'bottom-right', 'center'] as const).map((position) => (
          ...
          >
            <span className="text-[10px] font-sans uppercase tracking-widest text-slate-500 dark:text-zinc-400">
              {position.replace('-', ' ')}
            </span>
          </button>
        ))}
