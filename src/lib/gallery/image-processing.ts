...
function watermarkPosition(
  ...
  switch (position) {
    ...
    case "top-right":
      return { x: width - padding - textWidth, y: padding + textHeight };
    case "bottom-left":
      return { x: padding, y: height - padding };
    case "center":
      return {
        x: Math.max(0, (width - textWidth) / 2),
        y: Math.max(textHeight, (height + textHeight) / 2),
      };
    case "bottom-right":
    default:
...
async function createWatermarkOverlay(
  ...
  const fontSize = Math.max(12, Math.min(160, options.fontSize));
  const text = escapeXml(options.text || "KIPSMTHN");
  const textWidth = Math.max(fontSize * 3, text.length * fontSize * 0.58);
...
export async function processImage(
  ...
  if (watermarkOptions.enabled && watermarkOptions.text.trim()) {
    const displayMeta = await sharp(display).metadata();
    const watermarkWidth = displayMeta.width || Math.min(width || 2400, 2400);
    const watermarkHeight = displayMeta.height || Math.min(height || 2400, 2400);
...
export async function renderDownloadVariant(
  ...
  if (options.maxWidth) {
    image = image.resize({
      width: Math.max(320, Math.min(options.maxWidth, 8000)),
  ...
  if (options.watermark?.enabled && options.watermark.text.trim()) {
    ...
    const width = meta.width || options.maxWidth || 2400;
    const height = meta.height || 1600;
    const overlay = await createWatermarkOverlay(width, height, options.watermark);
    image = image.composite([{ input: overlay, blend: "over" }]);
  }

  const quality = Math.max(40, Math.min(options.quality || 90, 100));
