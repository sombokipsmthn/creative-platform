...
function getProvider() {
  return (process.env.STORAGE_PROVIDER || "vercel-blob").toLowerCase();
}
...
function r2PublicUrl(path: string) {
  ...
  return publicBase ? `${publicBase}/${path}` : `/api/public/galleries/media?key=${encodeURIComponent(path)}`;
}

class VercelBlobStorage implements GalleryStorage {
  ...
  async deleteObject(pathOrUrl: string) {
...
class R2Storage implements GalleryStorage {
  async putObject(input: {
    path: string;
    body: Buffer | Uint8Array;
    contentType: string;
  }): Promise<StorageObject> {
    const { token } = getR2Config();
    const response = await fetch(r2ObjectUrl(input.path), {
      method: "PUT",
      headers: {
  ...
  async deleteObject(pathOrUrl: string) {
    const path = pathOrUrl.startsWith("http")
      ? new URL(pathOrUrl).pathname.split("/objects/")[1]
      : pathOrUrl;

    if (!path) return;

    const { token } = getR2Config();
    const response = await fetch(r2ObjectUrl(decodeURIComponent(path)), {
      method: "DELETE",
  ...
  async getObject(path: string) {
    const { token } = getR2Config();
    const response = await fetch(r2ObjectUrl(path), {
      headers: { Authorization: `Bearer ${token}` },
      cache: "no-store",
    });
...
export function getGalleryStorage(): GalleryStorage {
  return getProvider() === "r2" ? new R2Storage() : new VercelBlobStorage();
}
